...
// ============================================
// SKELETON: Grid card (matches real card padding, image, buttons)
// ============================================
const PublicationSkeletonGridCard = () => (
  <div className="text-black flex h-full min-w-0 flex-col rounded-2xl bg-white p-4 sm:p-5 md:p-6 lg:p-5 xl:p-7.5 shadow-sm">
    {/* Image */}
    <Skeleton className="mb-4 sm:mb-5 h-40 sm:h-48 md:h-56 lg:h-52 xl:h-62.5 w-full rounded-2xl" />

    <div className="flex-1">
      {/* Date */}
      <Skeleton className="h-3.5 w-24 mb-2" />
      {/* Title (2 lines) */}
      <Skeleton className="h-5 sm:h-6 md:h-7 w-5/6 mb-2" />
      <Skeleton className="h-5 sm:h-6 md:h-7 w-3/5 mb-4" />
      {/* Excerpt (4 lines) */}
...
// ============================================
// SKELETON: List row (horizontal layout)
// ============================================
const PublicationSkeletonListCard = () => (
  <div className="shadow-sm p-4 sm:p-5 md:p-6 lg:p-7.5 rounded-2xl bg-white flex flex-col md:flex-row gap-4 sm:gap-5 md:gap-6">
    {/* Thumbnail */}
    <Skeleton className="w-full md:w-40 lg:w-48 xl:w-56 h-32 sm:h-40 md:h-44 lg:h-48 rounded-2xl shrink-0" />

    <div className="flex-1 flex flex-col justify-between">
      <div>
        <Skeleton className="h-3.5 w-24 mb-2" />
        <Skeleton className="h-5 sm:h-6 w-3/4 mb-2" />
        <SkeletonText lines={3} lineClassName="h-3.5 sm:h-4" />
      ...
      <div className="mt-4 flex flex-wrap gap-2 sm:gap-3">
        <Skeleton className="h-8 sm:h-9 w-28 sm:w-32 rounded-xl" />
        <Skeleton className="h-8 sm:h-9 w-32 sm:w-40 rounded-xl" />
      </div>
    </div>
  </div>
);
...
// ============================================
// SKELETON: Full section (header + filters + grid/list)
// ============================================
const PublicationsSkeleton = ({ viewMode = 'grid', count = 6 }) => (
  <>
    {/* Filters bar skeleton (only when not related) */}
    <div className="flex flex-col sm:flex-row flex-wrap items-start sm:items-center justify-between gap-3 sm:gap-4 pt-4 sm:pt-6 md:pt-8 lg:pt-10">
    ...
    {/* Grid / List */}
    {viewMode === 'grid' ? (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 md:gap-6 lg:gap-5 xl:gap-7.5 items-stretch pt-8 sm:pt-10 md:pt-12 lg:pt-15">
        {Array.from({ length: count }).map((_, i) => (
          <PublicationSkeletonGridCard key={`pub-grid-skel-${i}`} />
        ))}
      </div>
    ) : (
      <div className="text-black flex flex-col gap-4 sm:gap-5 md:gap-6 pt-8 sm:pt-10 md:pt-12 lg:pt-15">
        {Array.from({ length: count }).map((_, i) => (
          <PublicationSkeletonListCard key={`pub-list-skel-${i}`} />
        ))}
      </div>
    )}
  </>
...
const PublicationsSection = ({
  ...
  if (hasValue(resolvedData)) {
    ...
    if (Array.isArray(resolvedData)) {
      ...
      const featuredPub = resolvedData.find(
        (pub) => pub.is_featured === true || pub.is_featured === 1
      );
      if (featuredPub) resolvedMainPublication = featuredPub;
      else if (resolvedData.length > 0) resolvedMainPublication = resolvedData[0];
    } else if (typeof resolvedData === 'object') {
      if (hasValue(resolvedData.mainPublication)) resolvedMainPublication = resolvedData.mainPublication;
      else if (hasValue(resolvedData.main)) resolvedMainPublication = resolvedData.main;
      else if (hasValue(resolvedData.featured)) resolvedMainPublication = resolvedData.featured;

      if (Array.isArray(resolvedData.publicationItems)) resolvedPublicationItems = resolvedData.publicationItems;
      else if (Array.isArray(resolvedData.items)) resolvedPublicationItems = resolvedData.items;
      else if (Array.isArray(resolvedData.results)) resolvedPublicationItems = resolvedData.results;
      else if (Array.isArray(resolvedData.publications)) resolvedPublicationItems = resolvedData.publications;
  ...
  const hasMainPublication =
    !isRelatedSection &&
    hasValue(resolvedMainPublication) &&
    hasValue(resolvedMainPublication.title) &&
    hasValue(resolvedMainPublication.image);

  const hasPublicationItems =
    hasValue(resolvedPublicationItems) && resolvedPublicationItems.length > 0;

  // ============================================
  // LOADING STATE
  // ============================================
  if (loading) {
    // Related sections show title left-aligned, then 3-item grid
    if (isRelatedSection) {
      return (
        <section
          id={sectionId}
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 md:gap-6 lg:gap-5 xl:gap-7.5 items-stretch">
            {Array.from({ length: 3 }).map((_, i) => (
              <PublicationSkeletonGridCard key={`pub-related-skel-${i}`} />
            ))}
          </div>
        </section>
      );
    ...
    return (
      <section
        ...
        className={`${bgColor} ${paddingX} ${paddingY} ${sectionClassName}`}
      >
        <PublicationsSkeleton viewMode={viewMode} count={skeletonCount} />
      </section>
    );
  }
  ...
  const getDetailUrl = (publication) => {
    if (publication.slug) return `/publications/${publication.slug}`;
    if (publication.link) return publication.link;
    if (publication.id) return `/publications/${publication.id}`;
    return '#';
  };
  ...
  const getSortedPublications = () => {
    ...
    if (isRelatedSection) return sorted;
    switch (selectedSort) {
      case 'Newest':
  ...
  const renderListView = () => (
    <div
      className={`text-black flex flex-col gap-4 sm:gap-5 md:gap-6 ${hasMainPublication ? 'pt-8 sm:pt-10 md:pt-12 lg:pt-15' : ''
        }`}
    >
      {sortedPublications.map((publication) => {
        if (hasMainPublication && resolvedMainPublication.id === publication.id) {
        ...
        return (
          <div
            key={publication.id}
            className="shadow-2xl p-4 sm:p-5 md:p-6 lg:p-7.5 rounded-2xl hover:shadow-3xl transition-shadow duration-300 bg-white flex flex-col md:flex-row gap-4 sm:gap-5 md:gap-6"
          >
            {hasValue(publication.image) && (
              <img
                src={publication.image}
    ...
    >
      ...
      {hasPublicationItems && (
        <>
          {!isRelatedSection && (
            <div
              className={`flex flex-col sm:flex-row flex-wrap items-start sm:items-center justify-between gap-3 sm:gap-4 ${hasMainPublication ? 'pt-4 sm:pt-6 md:pt-8 lg:pt-10' : ''
                }`}
            >
              ...
              <div className="flex items-center flex-wrap gap-2">
                <div className="relative">
                  ...
                  >
                    <p className="text-[#5D6174] text-[12px] sm:text-[13px] md:text-[14px] pr-2 hidden sm:block">
                      Sort by:
                    </p>
                  ...
                  {isSortOpen && (
                    <div className="absolute top-full right-0 sm:left-0 mt-2 bg-white border border-[#E8E8EB] rounded-xl shadow-lg py-2 min-w-40 sm:min-w-45 md:min-w-50 z-10">
                      {sortOptions.map((option) => (
                        <div
                          key={option}
                          className={`px-3 sm:px-4 py-1.5 sm:py-2 text-[12px] sm:text-[13px] md:text-[14px] hover:bg-[#F6F6F7] cursor-pointer transition-colors ${selectedSort === option
                              ? 'text-[#009BE2] font-medium'
                              : 'text-[#171D38]'
                            }`}
                ...
                <div className="px-1.5 sm:px-2 py-1 rounded-xl bg-[#F6F6F7] border border-[#E8E8EB] gap-1 flex items-center">
                  ...
                  >
                    <LayoutGrid size={14} color={viewMode === 'grid' ? 'white' : '#171D38'} />
                  </div>
                  <div
                  ...
                  >
                    <List size={15} color={viewMode === 'list' ? 'white' : '#171D38'} />
                  </div>
                </div>
              </div>
            </div>
          ...
          {sortedPublications.length === 0 && (
            <div className="text-center py-10">
              <p className="text-gray-500 text-[15px] sm:text-[16px] md:text-[17px] lg:text-[18px]">
