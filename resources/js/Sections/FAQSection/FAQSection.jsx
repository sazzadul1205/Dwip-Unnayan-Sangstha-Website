...
// ============================================
// SKELETON: Single FAQ row (collapsed state)
// ============================================
const FAQSkeletonRow = () => (
  <div className="bg-white w-full rounded-lg overflow-hidden shadow-sm">
    <div className="w-full py-3.5 sm:py-4 md:py-5 lg:py-6 xl:py-7.5 px-4 sm:px-5 md:px-6 lg:px-7.5">
      <div className="flex items-start sm:items-center justify-between gap-3 sm:gap-4">
        {/* Question — variable width so it looks organic */}
        <Skeleton className="h-4 sm:h-4.5 md:h-5 lg:h-5.5 flex-1 max-w-[85%]" />
        {/* Plus/minus */}
        <Skeleton className="h-5 sm:h-5.5 md:h-6 lg:h-6.5 w-5 sm:w-5.5 md:w-6 lg:w-6.5 rounded-md shrink-0" />
      </div>
    </div>
  </div>
);
...
// ============================================
const FAQSkeleton = ({ count = 6, hasTitle = true, hasSubtitle = true }) => (
  <>
    {/* Header */}
    <div className="max-w-4xl lg:max-w-6xl xl:max-w-7xl mx-auto text-center">
      {hasTitle && (
        <Skeleton className="h-8 sm:h-9 md:h-10 lg:h-11 xl:h-12 2xl:h-13 w-3/4 sm:w-1/2 mx-auto mb-3 sm:mb-4" />
      )}
      {hasSubtitle && (
        <Skeleton className="h-4 sm:h-4.5 md:h-5 lg:h-5.5 w-full sm:w-4/5 md:w-2/3 mx-auto mb-1" />
      )}
    </div>
    ...
    <div className="mt-6 sm:mt-8 md:mt-10 lg:mt-12 xl:mt-15 space-y-2 sm:space-y-2.5 max-w-4xl lg:max-w-6xl xl:max-w-7xl mx-auto">
      {Array.from({ length: count }).map((_, i) => (
        <FAQSkeletonRow key={`faq-skeleton-${i}`} />
      ))}
    </div>
  </>
);
...
const FAQSection = ({
  ...
  if (loading) {
    return (
      ...
      >
        <FAQSkeleton count={skeletonCount} hasTitle hasSubtitle />
      </section>
    );
  }
  ...
  const validFaqs = faqs.filter(
    (faq) => hasValue(faq.question) || hasValue(faq.answer)
  );
              ...
              >
                {hasQuestion && (
                  ...
                  >
                    <div className="flex items-start sm:items-center justify-between gap-3 sm:gap-4">
                      <h3
                        className={`font-600 text-[15px] sm:text-[16px] md:text-[18px] lg:text-[19px] xl:text-[20px] leading-tight flex-1 transition-colors duration-300 ${isOpen ? 'text-[#009BE2]' : 'text-[#080C14]'
                          }`}
                      >
                        {faq.question}
                      </h3>
