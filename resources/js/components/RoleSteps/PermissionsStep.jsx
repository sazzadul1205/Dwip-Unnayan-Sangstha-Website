...
export const PermissionsStep = ({ formData, setFormData, permissions }) => {
  ...
  const handlePermissionToggle = (permissionId) => {
    // ✅ FIX: Ensure we're working with the latest state and merging properly
    setFormData((prev) => {
  ...
  const selectAllModule = (modulePermissions) => {
    ...
    setFormData((prev) => {
      ...
      return {
        ...prev,
        permissions: [...new Set([...currentPermissions, ...allIds])]
      };
    });
  };
  ...
  const deselectAllModule = (modulePermissions) => {
    ...
    setFormData((prev) => {
      ...
      return {
        ...prev,
        permissions: currentPermissions.filter(id => !allIds.includes(id))
      };
    });
  };
  ...
  // Debug log
  console.log('PermissionsStep - formData:', formData);
  console.log('PermissionsStep - selectedCount:', selectedCount);
      ...
      <div className="space-y-6">
        ...
        {filteredPermissions.length === 0 ? (
          <div className="text-center py-12">
            <FaKey className="text-gray-400 mx-auto mb-4" size={48} />
                      ...
                      <button onClick={() => toggleModule(module.module)} className="flex items-center gap-3 flex-1 text-left">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            {isAllSelected && (
                              <span className="text-xs text-green-600 bg-green-50 px-2 py-0.5 rounded-full">All selected</span>
                            )}
                            {isSomeSelected && (
                  ...
                  {isExpanded && (
                    <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-2">
                      {module.permissions.map(permission => (
                        <label
                          key={permission.id}
                          className={`flex items-start gap-3 p-3 rounded-lg cursor-pointer transition-colors ${(formData.permissions || []).includes(permission.id) ? 'bg-purple-50 border border-purple-200' : 'hover:bg-gray-50 border border-transparent'}`}
                        >
                          <input
                            type="checkbox"
