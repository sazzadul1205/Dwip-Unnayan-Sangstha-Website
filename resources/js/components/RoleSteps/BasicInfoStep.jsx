...
export const BasicInfoStep = ({
  ...
  return (
    <StepWrapper title="Basic Information" description={isEdit ? "Edit the fundamental details of the role" : "Enter the fundamental details of the role"} isActive={true} stepNumber={1}>
      <div className="space-y-6">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Role Name <span className="text-red-500">*</span></label>
          <input type="text" name="name" value={formData.name} onChange={handleChange} onBlur={generateSlug}
            placeholder="e.g., Content Manager, Sales Lead, Developer" disabled={isDefaultRole}
            className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition ${errors.name ? 'border-red-500' : 'border-gray-300'} ${isDefaultRole ? 'bg-gray-100 cursor-not-allowed' : ''}`} />
          {errors.name && <p className="mt-1 text-sm text-red-500">{errors.name}</p>}
          {isDefaultRole && <p className="mt-1 text-xs text-amber-600">Default role name cannot be changed</p>}
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input type="text" name="slug" value={formData.slug} onChange={handleSlugChange} placeholder="e.g., content-manager" disabled={isDefaultRole}
              className={`w-full pl-8 pr-4 py-2 border rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent font-mono text-sm ${errors.slug ? 'border-red-500' : 'border-gray-300'} ${isDefaultRole ? 'bg-gray-100 cursor-not-allowed' : ''}`} />
          </div>
        ...
        <div>
          <div className="flex justify-between items-center mb-2">
            <label className="block text-sm font-medium text-gray-700">Access Level <span className="text-red-500">*</span></label>
            <button type="button" onClick={applyLevelRecommendation} className="text-xs text-purple-600 hover:text-purple-800 flex items-center gap-1"><FaMagic size={10} /> Suggest</button>
          </div>
          <div className="flex gap-4 items-center">
            <input type="number" name="level" value={formData.level} onChange={handleChange} placeholder="1-100" min="1" max="100"
            ...
            <div className="flex-1">
              <div className="relative">
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div className="bg-linear-to-r from-purple-500 to-indigo-500 rounded-full h-2 transition-all duration-300" style={{ width: `${(formData.level / 100) * 100}%` }} />
                </div>
          ...
          {existingLevels && existingLevels.length > 0 && (
            <div className="mt-4 p-3 bg-gray-50 rounded-lg">
              <p className="text-xs font-medium text-gray-600 mb-2 flex items-center gap-1"><FaInfoCircle size={10} /> Existing Role Levels Reference:</p>
              <div className="flex flex-wrap gap-2">
                {existingLevels.slice(0, 10).map(role => (
                  <span key={role.level} className="text-xs px-2 py-1 bg-gray-200 rounded-full text-gray-600">Lvl {role.level}: {role.name}</span>
                ))}
                {existingLevels.length > 10 && <span className="text-xs text-gray-400">+{existingLevels.length - 10} more</span>}
              </div>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
          <label className="flex items-center justify-between p-4 bg-gray-50 rounded-xl cursor-pointer hover:bg-gray-100 transition">
            <div className="flex items-center gap-3">
              ...
              <div><span className="text-sm font-medium text-gray-700">Active</span><p className="text-xs text-gray-400">Inactive roles cannot be assigned to users</p></div>
            </div>
            <input type="checkbox" name="is_active" checked={formData.is_active} onChange={handleChange} className="w-5 h-5 text-green-600 border-gray-300 rounded focus:ring-green-500" />
          </label>
          ...
          <label className={`flex items-center justify-between p-4 bg-gray-50 rounded-xl transition ${!isDefaultRole ? 'cursor-pointer hover:bg-gray-100' : 'cursor-not-allowed opacity-75'}`}>
            <div className="flex items-center gap-3">
              ...
              <div><span className="text-sm font-medium text-gray-700">Default Role</span><p className="text-xs text-gray-400">{isDefaultRole ? 'Default roles cannot be changed' : 'Auto-assigned to new users (only one default role allowed)'}</p></div>
            </div>
            <input type="checkbox" name="is_default" checked={formData.is_default} onChange={handleChange} disabled={isDefaultRole} className="w-5 h-5 text-purple-600 border-gray-300 rounded focus:ring-purple-500 disabled:opacity-50" />
          </label>
        </div>
