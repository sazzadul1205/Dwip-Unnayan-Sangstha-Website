...
// (or if you keep it as ProjectsAndProgramsDetails, update accordingly)
...
// ------------------------------------------------------------------
const PublicationBannerSection = ({ bannerData, publicationData, notFound }) => {
  if (notFound) {
    return (
      <section className="relative isolate w-full h-125 overflow-hidden bg-[#080C14] flex items-center justify-center">
        <div className="text-white text-center max-w-2xl px-4">
          ...
          <p className="text-lg text-gray-300">The publication you're looking for is no longer available or has been removed.</p>
  ...
  const normalizedData = {
    ...
    createdBy: publicationData?.createdBy || publicationData?.author || 'ADMIN',
    ...
    timerRead: publicationData?.timerRead || publicationData?.read_time || '5 MIN READ',
    tags: publicationData?.tags || [],
  };

  const defaultBanner = {
    background: {
      src: publicationData?.image || '/storage/OurPrograms/db1b2b6eae5fc260b4204f8257dadbd5a7aa0af7.png',
    ...
    <section className="relative isolate w-full h-125 overflow-hidden bg-[#080C14]">
      <div className="absolute inset-0 z-0">
        {banner?.background?.src && (
          <img
            ...
            alt={banner.background.alt || `Banner for ${normalizedData.title}`}
            loading="lazy"
      ...
      <div className="relative z-10 max-w-275 mx-auto px-4 pt-24 sm:pt-28 lg:pt-32 h-full flex flex-col items-center justify-start text-center">
        <div className="flex items-center justify-center gap-2.5 flex-wrap mb-5">
          {normalizedData.tags?.length > 0 ? (
            normalizedData.tags.map((tag, index) => {
              const tagColors = [
        ...
        <div className="flex items-center justify-center gap-4 sm:gap-6 flex-wrap text-white text-[12px] sm:text-[14px] font-semibold">
          <div className="flex items-center gap-2.5">
            ...
            <p className="flex items-center">
              BY : <span className="pl-1">{normalizedData.createdBy}</span>
            </p>
          </div>

          <div className="flex items-center gap-2">
            <CiCalendar className="text-base" />
            <span>{normalizedData.date}</span>
          </div>

          <div className="flex items-center gap-2">
            <FaRegClock className="text-base" />
            <span>{normalizedData.timerRead}</span>
          </div>
        </div>
      </div>
    </section>
...
const PublicationContentSection = ({
  ...
  const imageUrl = getImageUrl(normalizedData.image);
        ...
        <div className="-mt-16 sm:-mt-20 lg:-mt-24">
          {imageUrl ? (
            <div className="aspect-video w-full overflow-hidden rounded-[28px] shadow-2xl">
              <img
                src={imageUrl}
                alt={`Featured image for ${normalizedData.title}`}
                loading="lazy"
        ...
        <div className="flex flex-col lg:flex-row items-start gap-8 lg:gap-25">
          ...
          <div className="flex-1">
            <div
              ...
              dangerouslySetInnerHTML={renderHTML(normalizedData.fullContent)}
            />
          </div>
        </div>
      </div>
...
// Not Found Component
// ------------------------------------------------------------------
const NotFoundContent = ({ icon = "📄", title = "Content Not Found", message = "The publication you're looking for is no longer available or has been removed.", buttonText = "Return to Home", buttonLink = "/" }) => (
...
const ProjectsAndProgramsDetails = ({
  ...
  const publicationData = actualPageData.publicationData || actualPageData.contentSectionData || null;
  ...
  if (serverNotFound) {
    return (
      ...
      >
        <Head title="Publication Not Found | DUS" />
        <NotFoundContent
          icon="📝"
          title="Publication Not Available"
          message={notFoundMessage || 'The publication you are looking for is no longer available or has been removed.'}
  ...
  const renderSpecialComponent = (section) => {
    ...
    if (component === 'BannerSection' || component === 'PageBannerSection') {
      return (
        <PublicationBannerSection
          key={section.id}
          bannerData={bannerData}
          publicationData={publicationData}
          notFound={!publicationData}
          {...customProps}
        />
      );
    }

    if (component === 'PublicationContentSection' || component === 'ProgramContentSection') {
      return (
