...
export default function RolesTrashed({ roles: initialRoles, filters: initialFilters = {}, stats: initialStats = {} }) {
  ...
  useEffect(() => {
    if (flash?.success) Swal.fire({ icon: 'success', title: 'Success!', text: flash.success, timer: 2000, showConfirmButton: false });
    if (flash?.error) Swal.fire({ icon: 'error', title: 'Error!', text: flash.error });
  }, [flash]);
        ...
        <div className="mx-auto">
          <div className="flex justify-between items-start mb-6 animate-fade-in">
            <div>
              ...
              <h1 className="text-3xl font-bold bg-linear-to-r from-gray-900 to-gray-600 bg-clip-text text-transparent">Trashed Roles</h1>
          ...
          {selectedRoles.length > 0 && (
            <div className="bg-linear-to-r from-amber-50 to-orange-50 rounded-xl shadow-lg p-4 mb-6 border border-amber-200 animate-fade-in">
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div className="flex items-center gap-3"><FaCheckDouble className="text-amber-600" size={20} /><span className="font-semibold text-gray-900">{selectedRoles.length} role(s) selected</span></div>
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setSelectedRoles([])} className="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition">Clear Selection</button>
                </div>
              </div>
            </div>
          )}
          ...
          {showFilters && (
            <div className="bg-white rounded-xl shadow-lg p-6 mb-6 animate-fade-in">
              <div className="flex justify-between items-center mb-4"><h3 className="text-lg font-semibold text-gray-900">Filter Trashed Roles</h3><button onClick={resetFilters} className="text-sm text-red-600 hover:text-red-800 flex items-center gap-1"><FaTimes size={12} /> Reset all</button></div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div><label className="block text-sm font-medium text-gray-700 mb-2">Search</label><div className="relative"><FaSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={14} /><input type="text" value={filters.search} onChange={(e) => handleFilterChange('search', e.target.value)} placeholder="Search by name, slug..." className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" /></div></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-2">Sort By</label><select value={filters.sortBy} onChange={(e) => handleFilterChange('sortBy', e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"><option value="deleted_at">Deleted Date</option><option value="name">Role Name</option><option value="level">Access Level</option></select></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-2">Sort Direction</label><select value={filters.sortDir} onChange={(e) => handleFilterChange('sortDir', e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"><option value="desc">Descending</option><option value="asc">Ascending</option></select></div>
              </div>
            </div>
          )}
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-linear-to-r from-gray-50 to-gray-100">
                  <tr>
                    <th className="px-4 py-4 text-left"><input type="checkbox" checked={selectedRoles.length === sortedRoles.filter(r => canRestoreSpecificRole(r)).length && sortedRoles.filter(r => canRestoreSpecificRole(r)).length > 0} onChange={handleSelectAll} className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" disabled={sortedRoles.filter(r => canRestoreSpecificRole(r)).length === 0} /></th>
                    ...
                    <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Deleted Info</th>
                    <th className="px-6 py-4 text-right text-xs font-semibold text-gray-600 uppercase tracking-wider">Actions</th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                  {sortedRoles.length === 0 && (
                    <tr><td colSpan="5" className="text-center py-16"><FaTrash className="text-gray-400 mx-auto mb-4" size={48} /><p className="text-gray-500">No trashed roles found.</p></td></tr>
                  )}
                  {sortedRoles.map(role => {
                    ...
                    return (
                      <tr key={role.id} className={`hover:bg-gray-50 transition-all duration-200 animate-fade-in ${selectedRoles.includes(role.id) ? 'bg-amber-50' : ''}`}>
                        <td className="px-4 py-4">{canRestore && <input type="checkbox" checked={selectedRoles.includes(role.id)} onChange={() => handleSelectRole(role.id)} className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />}</td>
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-lg bg-gray-200 flex items-center justify-center shrink-0"><FaShieldAlt className="text-gray-500" size={18} /></div>
                            <div>
                              <div className="font-semibold text-gray-900 line-through">{role.name}{isHighLevelRestricted && <span className="ml-2 inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium bg-yellow-100 text-yellow-700"><FaLock size={10} /> Restricted</span>}</div>
                        ...
                        <td className="px-6 py-4"><span className={`inline-flex px-3 py-1.5 rounded-full text-sm font-bold ${getLevelBadge(role.level)}`}>Level {role.level}</span></td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right">
                          <div className="flex justify-end gap-2">
                            ...
                            <Can permission="roles.restore" fallback={null}>
                              <button onClick={() => handleRestore(role.id, role.name)} disabled={restoringId === role.id || !canRestore} className={`p-2 rounded-lg transition-all duration-200 ${canRestore ? 'text-green-600 hover:text-green-900 hover:bg-green-50' : 'text-gray-400 cursor-not-allowed'} ${restoringId === role.id ? 'opacity-50 cursor-not-allowed' : ''}`} title={!canRestore ? 'You don\'t have permission to restore this role' : 'Restore'}>
                            ...
                            <Can permission="roles.force_delete" fallback={null}>
                              <button onClick={() => handleForceDelete(role.id, role.name)} disabled={forceDeletingId === role.id || !canForceDelete} className={`p-2 rounded-lg transition-all duration-200 ${canForceDelete ? 'text-red-600 hover:text-red-900 hover:bg-red-50' : 'text-gray-400 cursor-not-allowed'} ${forceDeletingId === role.id ? 'opacity-50 cursor-not-allowed' : ''}`} title={!canForceDelete ? (role.is_default ? 'Default roles cannot be permanently deleted' : 'You don\'t have permission to permanently delete this role') : 'Permanently Delete'}>
