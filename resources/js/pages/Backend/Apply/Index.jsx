...
export default function ApplyIndex({ applications: initialApplications, stats: initialStats }) {
  ...
  const Pagination = () => {
    ...
    return (
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4 px-3 sm:px-5 py-3 sm:py-4 border-t border-gray-200 bg-gray-50 rounded-b-xl">
        ...
        <div className="flex flex-wrap items-center justify-center gap-1">
          <button
            ...
            className={`px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-lg text-xs sm:text-sm flex items-center gap-0.5 sm:gap-1 transition ${
              pagination.currentPage === 1
                ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
                : 'bg-white text-gray-600 hover:bg-gray-100 border border-gray-300'
            }`}
          >
            <FaChevronLeft size={10} className="sm:w-3 sm:h-3" /> <span className="hidden xs:inline">Prev</span>
          </button>
          
          {startPage > 1 && (
            <>
              <button onClick={() => handlePageChange(1)} className="px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-lg text-xs sm:text-sm bg-white text-gray-600 hover:bg-gray-100 border border-gray-300">1</button>
              {startPage > 2 && <span className="px-1 sm:px-2 text-gray-400 text-xs sm:text-sm">...</span>}
            </>
          )}
          ...
          {endPage < pagination.lastPage && (
            <>
              {endPage < pagination.lastPage - 1 && <span className="px-1 sm:px-2 text-gray-400 text-xs sm:text-sm">...</span>}
              <button onClick={() => handlePageChange(pagination.lastPage)} className="px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-lg text-xs sm:text-sm bg-white text-gray-600 hover:bg-gray-100 border border-gray-300">
                {pagination.lastPage}
              </button>
            </>
          )}
          ...
          >
            <span className="hidden xs:inline">Next</span> <FaChevronRight size={10} className="sm:w-3 sm:h-3" />
          </button>
        </div>
      </div>
    );
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="min-h-screen bg-linear-to-br from-gray-50 to-gray-100/50 p-3 sm:p-4 md:p-6">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
            ...
            <div className="flex flex-wrap gap-2 w-full sm:w-auto">
              <button
                ...
                className={`flex-1 sm:flex-none px-3 sm:px-4 py-2 rounded-lg flex items-center justify-center gap-2 transition-all duration-200 text-xs sm:text-sm font-medium ${
                  showTrashed
              ...
              {!showTrashed && (
                <Link
                  ...
                  className="flex-1 sm:flex-none bg-linear-to-r from-blue-600 to-blue-700 text-white px-3 sm:px-4 py-2 rounded-lg flex items-center justify-center gap-2 hover:from-blue-700 hover:to-blue-800 transition-all shadow-sm text-xs sm:text-sm font-medium"
                >
                  <FaPlus size={12} />
                  Browse Jobs
                </Link>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-2 sm:gap-3 mb-4 sm:mb-6">
            {statsCards.map((card) => {
              ...
              return (
                <div
                  key={card.key}
                  className={`bg-linear-to-br ${colorMap[card.color]} rounded-xl p-2 sm:p-3 shadow-sm hover:shadow-md transition-all duration-200 cursor-pointer`}
                  onClick={() => {
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="opacity-80 text-xs sm:text-base">{card.icon}</div>
                    <span className="text-base sm:text-xl font-bold">{card.value}</span>
                  </div>
                  <p className="text-[10px] sm:text-xs font-medium mt-0.5 sm:mt-1 opacity-75">{card.title}</p>
                </div>
              );
            })}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3 sm:gap-5">
            ...
            {(showTrashed
              ...
              : applicationItems.filter(app => !app.deleted_at)
                ...
                <div className="col-span-full text-center py-8 sm:py-12 bg-white rounded-xl shadow-sm border border-gray-100">
                  <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-3">
                    <FaBriefcase className="text-gray-400 text-xl sm:text-2xl" />
                  </div>
                  ...
                  {!showTrashed && (
                    <Link
                      href={route('public.jobs.index')}
                      className="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-blue-600 text-white rounded-lg text-xs sm:text-sm font-medium hover:bg-blue-700 transition mt-3 sm:mt-4"
                    >
              ...
              return (
                <div
                  ...
                  className={`group bg-white rounded-xl shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden border ${
                    trashed ? 'border-gray-200 opacity-75' : 'border-gray-100'
                  }`}
                >
                  <div className="p-3 sm:p-4">
                    ...
                    <Link
                      ...
                      className={`font-semibold text-sm sm:text-base hover:text-blue-600 transition line-clamp-1 ${
                        trashed ? 'text-gray-500 line-through' : 'text-gray-900'
                      }`}
                    >
                      {app.job_title}
                    </Link>
                    ...
                    {isAdmin && !isOwnApplication && !trashed && (
                      <div className="mt-1">
                        <span className="text-[10px] sm:text-xs text-purple-600 bg-purple-50 px-1.5 sm:px-2 py-0.5 rounded-full">
                          Applicant: {app.name}
                        </span>
                      </div>
                    )}
                    ...
                    <div className="flex items-center justify-between mt-2 sm:mt-3">
                      ...
                      {!trashed && (
                        app.ats_score ? (
                          <div className={`inline-flex items-center gap-1 px-1.5 sm:px-2 py-0.5 rounded-full text-[10px] sm:text-xs font-medium ${getAtsScoreBg(app.ats_score)} ${getAtsScoreColor(app.ats_score)}`}>
                            <FaChartLine size={10} />
                            {app.ats_score}%
                          </div>
                        ) : isProcessing ? (
                          ...
                          <span className="text-[10px] sm:text-xs text-gray-400">ATS pending</span>
                        )
                      )}
                      ...
                      <div className="flex items-center gap-1 sm:gap-1.5">
                        {!trashed ? (
                          <>
                            ...
                            <span className={`text-[10px] sm:text-xs font-medium px-1.5 sm:px-2 py-0.5 rounded-full ${getStatusBadge(app.status)}`}>
                          ...
                          <>
                            ...
                            <span className="text-[10px] sm:text-xs font-medium bg-gray-100 text-gray-600 px-1.5 sm:px-2 py-0.5 rounded-full">Withdrawn</span>
                          </>
                        )}
                      </div>
                    ...
                    <div className="flex items-center justify-between mt-3 sm:mt-4 pt-2 sm:pt-3 border-t border-gray-100">
                      <div className="flex flex-wrap gap-0.5 sm:gap-1">
                        <Link
                          href={route('backend.apply.show', app.id)}
                          className="p-1.5 sm:p-1.5 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-lg transition"
                          title="View Details"
                        >
                        ...
                        {isPending && (
                          <Link
                            ...
                            className="p-1.5 sm:p-1.5 text-blue-500 hover:text-blue-700 hover:bg-blue-50 rounded-lg transition"
                            title="Edit"
                          >
                        ...
                        {isPending && (
                          <button
                            ...
                            className="p-1.5 sm:p-1.5 text-rose-500 hover:text-rose-700 hover:bg-rose-50 rounded-lg transition disabled:opacity-50"
                            title="Withdraw"
                          >
                            {withdrawingId === app.id ? <FaSpinner className="animate-spin" size={12} /> : <FaTrash size={12} className="sm:w-3.5 sm:h-3.5" />}
                          </button>
                        )}
                        {trashed && (
                          <>
                            <button
                              ...
                              className="p-1.5 sm:p-1.5 text-emerald-500 hover:text-emerald-700 hover:bg-emerald-50 rounded-lg transition"
                              title="Restore"
                            >
                              {restoringId === app.id ? <FaSpinner className="animate-spin" size={12} /> : <FaTrashRestore size={12} className="sm:w-3.5 sm:h-3.5" />}
                            </button>
                            <button
                              onClick={() => handleForceDelete(app.id, app.job_title)}
                              className="p-1.5 sm:p-1.5 text-rose-600 hover:text-rose-800 hover:bg-rose-50 rounded-lg transition"
                      ...
                      <div className="flex flex-wrap gap-0.5 sm:gap-1">
                        {app.resume_url && (
                          <a
                            ...
                            className="p-1.5 sm:p-1.5 text-purple-500 hover:text-purple-700 hover:bg-purple-50 rounded-lg transition"
                            title="View Resume"
                          >
                            <FaFilePdf size={12} className="sm:w-3.5 sm:h-3.5" />
                          </a>
                        )}
                        {!trashed && !isProcessing && (
                          <button
                            ...
                            className="p-1.5 sm:p-1.5 text-orange-500 hover:text-orange-700 hover:bg-orange-50 rounded-lg transition disabled:opacity-50"
                            title="Recalculate ATS"
                          >
                            {recalculatingId === app.id ? <FaSpinner className="animate-spin" size={12} /> : <FaChartLine size={12} className="sm:w-3.5 sm:h-3.5" />}
                          </button>
                        )}
                      </div>
                    </div>
