...
export default function ApplyEdit({ application, jobListing, cvs, currentCvId }) {
      ...
      <AuthenticatedLayout>
        ...
        <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
          <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-6 sm:p-8 text-center">
            ...
            <p className="text-sm sm:text-base text-gray-600 mb-6">Please login to edit your application.</p>
            <button
              onClick={() => router.visit(route('login', { redirect: route('backend.apply.edit', application.id) }))}
              className="w-full px-6 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-all duration-200 font-medium text-sm sm:text-base"
            >
              Login Now
            </button>
          </div>
      ...
      <AuthenticatedLayout>
        ...
        <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
          <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-6 sm:p-8 text-center">
            ...
            <p className="text-sm sm:text-base text-gray-600 mb-6">You don't have permission to edit this application.</p>
            <button
        ...
        <div className="min-h-screen bg-gray-50 py-8 sm:py-12 px-4">
          <div className="max-w-2xl mx-auto">
            <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
              <div className="bg-linear-to-r from-orange-500 to-red-500 px-4 sm:px-8 py-4 sm:py-6">
                <h1 className="text-xl sm:text-2xl font-bold text-white">Application Cannot Be Edited</h1>
              </div>
              <div className="p-6 sm:p-8 text-center">
                <div className="w-16 h-16 sm:w-20 sm:h-20 bg-orange-100 rounded-full flex items-center justify-center mx-auto mb-4">
                  <FaExclamationTriangle className="text-orange-500 text-2xl sm:text-3xl" />
                </div>
                ...
                <button
                  onClick={() => router.visit(route('backend.apply.show', application.id))}
                  className="px-6 sm:px-8 py-2.5 sm:py-3 bg-linear-to-r from-blue-600 to-indigo-600 text-white rounded-xl hover:from-blue-700 hover:to-indigo-700 transition-all duration-200 font-medium text-sm sm:text-base"
                >
                  Back to Application
                </button>
              </div>
      ...
      <div className="px-4 sm:px-6 py-3 sm:py-4 bg-linear-to-r from-purple-600 to-indigo-600">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div>
              <h3 className="font-semibold text-white text-sm sm:text-base">ATS Score Preview</h3>
              <p className="text-purple-200 text-[10px] sm:text-xs">New CV analysis</p>
            </div>
          </div>
          <button
      ...
      <div className="p-4 sm:p-6">
        {isLoadingAts ? (
          <div className="text-center py-8 sm:py-12">
            ...
            <p className="text-gray-600 mt-3 sm:mt-4 font-medium text-sm sm:text-base">Analyzing your CV...</p>
            <p className="text-gray-400 text-xs sm:text-sm">Comparing against job requirements</p>
          </div>
        ) : atsPreview?.error ? (
          <div className="text-center py-6 sm:py-8">
            ...
            <p className="text-gray-700 font-medium text-sm sm:text-base">{atsPreview.error}</p>
            <button
              onClick={handlePreviewAts}
              className="mt-3 sm:mt-4 text-xs sm:text-sm text-blue-600 hover:text-blue-800 font-medium transition"
            >
              Try Again →
            </button>
          </div>
          ...
          <div className="space-y-4 sm:space-y-5">
              ...
              <div className="relative inline-flex items-center justify-center w-24 h-24 sm:w-32 sm:h-32">
                <svg className="w-24 h-24 sm:w-32 sm:h-32 transform -rotate-90">
                  ...
                  <circle
                    ...
                    strokeWidth="6"
                    strokeDasharray={`${atsPreview.percentage * 2.51} 251`}
                    strokeLinecap="round"
                    className="transition-all duration-1000"
                  />
                </svg>
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="bg-white rounded-full w-16 h-16 sm:w-20 sm:h-20 flex items-center justify-center shadow-lg">
                    <span className={`text-xl sm:text-3xl font-bold ${getAtsScoreColor(atsPreview.percentage)}`}>
                      {atsPreview.percentage}%
                    </span>
                  </div>
                </div>
              </div>
              <p className="text-xs sm:text-sm text-gray-500 mt-1.5 sm:mt-2">Compatibility Score</p>
              {atsPreview.analysis && (
                <p className={`text-xs sm:text-sm font-medium mt-0.5 sm:mt-1 ${atsPreview.analysis.color === 'green' ? 'text-green-600' :
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
              <div className="text-center p-3 sm:p-4 bg-linear-to-br from-green-50 to-emerald-50 rounded-xl border border-green-100">
                <FaCheckCircle className="text-green-500 mx-auto mb-1.5 sm:mb-2" size={16} />
                <p className="text-green-600 font-medium text-xs sm:text-sm">Matched</p>
                <p className="text-lg sm:text-2xl font-bold text-green-700">{atsPreview.matched_count || 0}</p>
                <p className="text-[10px] sm:text-xs text-gray-500">keywords</p>
              </div>
              <div className="text-center p-3 sm:p-4 bg-linear-to-br from-red-50 to-rose-50 rounded-xl border border-red-100">
                <FaTimesCircle className="text-red-500 mx-auto mb-1.5 sm:mb-2" size={16} />
                <p className="text-red-600 font-medium text-xs sm:text-sm">Missing</p>
                <p className="text-lg sm:text-2xl font-bold text-red-700">{atsPreview.missing_count || 0}</p>
                <p className="text-[10px] sm:text-xs text-gray-500">keywords</p>
              </div>
            </div>
            ...
            {atsPreview.top_matched?.length > 0 && (
              <div>
                <p className="text-[10px] sm:text-xs font-semibold text-green-600 mb-1.5 sm:mb-2 flex items-center gap-1">
                  <FaCheckCircle size={10} /> Top Matched Keywords
                </p>
                <div className="flex flex-wrap gap-1.5 sm:gap-2">
                  {atsPreview.top_matched.slice(0, 6).map((keyword, idx) => (
                    <span key={idx} className="px-2 sm:px-3 py-0.5 sm:py-1 text-[10px] sm:text-xs bg-green-100 text-green-700 rounded-full font-medium">
                      {keyword}
                    </span>
                  ))}
                </div>
            ...
            {atsPreview.top_missing?.length > 0 && (
              <div>
                <p className="text-[10px] sm:text-xs font-semibold text-red-600 mb-1.5 sm:mb-2 flex items-center gap-1">
                  <FaTimesCircle size={10} /> Missing Keywords to Add
                </p>
                <div className="flex flex-wrap gap-1.5 sm:gap-2">
                  {atsPreview.top_missing.slice(0, 6).map((keyword, idx) => (
                    <span key={idx} className="px-2 sm:px-3 py-0.5 sm:py-1 text-[10px] sm:text-xs bg-red-100 text-red-700 rounded-full font-medium">
                      {keyword}
                    </span>
                  ))}
                </div>
            ...
            {atsPreview.analysis?.suggestions?.length > 0 && (
              <div className="mt-3 sm:mt-4 p-3 sm:p-4 bg-linear-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-100">
                <div className="flex items-center gap-1.5 sm:gap-2 mb-1.5 sm:mb-2">
                  <FaLightbulb className="text-blue-600" size={14} />
                  <p className="text-xs sm:text-sm font-semibold text-blue-800">Suggestions to Improve</p>
                </div>
                <ul className="space-y-1">
                  {atsPreview.analysis.suggestions.slice(0, 3).map((suggestion, idx) => (
                    <li key={idx} className="text-xs sm:text-sm text-blue-700 flex items-start gap-1.5 sm:gap-2">
    ...
    return (
      <AuthenticatedLayout>
        ...
        <div className="min-h-screen bg-gray-50 py-8 sm:py-12 px-4">
          <div className="max-w-2xl mx-auto">
            <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
              <div className="bg-linear-to-r from-red-500 to-orange-500 px-4 sm:px-8 py-4 sm:py-6">
                <h1 className="text-xl sm:text-2xl font-bold text-white">Application Deadline Passed</h1>
              </div>
              <div className="p-6 sm:p-8 text-center">
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="min-h-screen bg-linear-to-br from-gray-50 to-gray-100 py-4 sm:py-6 px-3 sm:px-6 lg:px-8">
        <div className="mx-auto">
          {/* Back Button */}
          <button
            onClick={() => window.history.back()}
            className="group inline-flex items-center gap-1.5 sm:gap-2 text-gray-600 hover:text-gray-900 mb-4 sm:mb-6 px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl hover:bg-white/60 transition-all duration-200 text-xs sm:text-sm"
          >
          ...
          {/* Header */}
          <div className="bg-white rounded-2xl shadow-lg overflow-hidden mb-6 sm:mb-8 animate-fade-in border border-gray-100">
                ...
                <div className="flex flex-col sm:flex-row items-start justify-between gap-3 sm:gap-4">
                  <div>
                    <div className="flex items-center gap-2 sm:gap-3 mb-1.5 sm:mb-2">
                      <div className="p-1.5 sm:p-2 bg-white/20 rounded-xl">
                        <FaPen className="text-white" size={14} />
                      </div>
                      <h1 className="text-lg sm:text-2xl font-bold text-white">Edit Application</h1>
                    </div>
                    ...
                    {!isOwner && canEditApplications && (
                      <p className="text-blue-200 text-[10px] sm:text-xs mt-1 sm:mt-2 flex items-center gap-1">
                        <FaShieldAlt size={10} />
                        Admin mode: Editing application for {application.name}
                      </p>
                    )}
                  </div>
                  <div className="px-3 sm:px-4 py-1.5 sm:py-2 bg-white/20 rounded-xl backdrop-blur-sm border border-white/30">
                    <span className="text-white text-xs sm:text-sm font-medium">Pending</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
            ...
            <div className="lg:col-span-2 space-y-4 sm:space-y-6">
              ...
              <div className="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100 hover:shadow-xl transition-shadow duration-300">
                <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-100 bg-linear-to-r from-gray-50 to-gray-100">
                  <div className="flex items-center gap-2">
                    <div className="p-1.5 sm:p-2 bg-blue-100 rounded-xl">
                      <FaBriefcase className="text-blue-600" size={14} />
                    </div>
                    <h2 className="font-semibold text-gray-900 text-sm sm:text-base">Job Summary</h2>
                  </div>
                </div>
                <div className="p-4 sm:p-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 text-xs sm:text-sm mb-3 sm:mb-4">
                    <div className="flex items-center gap-1.5 sm:gap-2 text-gray-600 bg-gray-50 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-xl">
                      <MdOutlineLocationOn className="text-blue-500" size={14} />
                    ...
                    <div className="flex items-center gap-1.5 sm:gap-2 text-gray-600 bg-gray-50 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-xl">
                      <FaDollarSign className="text-green-500" size={12} />
                      <span className="font-medium">{getSalaryDisplay()}</span>
                    </div>
                  </div>

                  <div className="inline-flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-xs sm:text-sm font-medium bg-blue-50 text-blue-700 border border-blue-100">
              ...
              <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100">
                <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-100 bg-linear-to-r from-gray-50 to-gray-100">
                  <div className="flex items-center gap-2">
                    <div className="p-1.5 sm:p-2 bg-indigo-100 rounded-xl">
                      <FaRegFileAlt className="text-indigo-600" size={14} />
                    </div>
                    <div>
                      <h2 className="font-semibold text-gray-900 text-sm sm:text-base">Application Information</h2>
                      <p className="text-[10px] sm:text-xs text-gray-500">Update your application details below</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
                  ...
                  <div>
                    ...
                    {cvs.length === 0 ? (
                      <div className="bg-yellow-50 border-2 border-yellow-200 rounded-2xl p-4 sm:p-6 text-center">
                        <div className="flex flex-col items-center">
                          <FaUpload className="text-yellow-600 text-2xl sm:text-3xl mb-2 sm:mb-3" />
                          <p className="text-yellow-800 font-medium text-sm sm:text-base mb-1.5 sm:mb-2">No CV found in your profile</p>
                          <a
                      ...
                      <div className="space-y-2 sm:space-y-3">
                        {cvs.map((cv) => {
                          ...
                          return (
                            <label
                              key={cv.id}
                              className={`group flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 sm:p-4 border-2 rounded-xl cursor-pointer transition-all duration-300 hover:shadow-md ${isSelected
                                ? willChangeCv
                            ...
                            >
                              <div className="flex items-center gap-3 sm:gap-4 flex-1 w-full sm:w-auto">
                                <input
                                  ...
                                  className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-blue-600 focus:ring-blue-500 shrink-0"
                                />
                                <div className="p-2 sm:p-3 bg-white rounded-xl shadow-sm shrink-0">
                                  <FaFilePdf className="text-red-500" size={18} />
                                </div>
                                <div className="min-w-0">
                                  <p className="text-xs sm:text-sm font-semibold text-gray-900 line-clamp-1">{cv.original_name}</p>
                                  <div className="flex flex-wrap items-center gap-1.5 sm:gap-3 mt-0.5 sm:mt-1">
                                    {cv.is_primary && (
                                      <span className="inline-flex items-center gap-0.5 sm:gap-1 text-[10px] sm:text-xs text-green-600 bg-green-50 px-1.5 sm:px-2 py-0.5 rounded-full">
                                        <FaCheckCircle size={8} /> Primary
                                      </span>
                                    )}
                                    {isCurrentCv && (
                                      <span className="inline-flex items-center gap-0.5 sm:gap-1 text-[10px] sm:text-xs text-blue-600 bg-blue-50 px-1.5 sm:px-2 py-0.5 rounded-full">
                                        <FaCheckCircle size={8} /> Current
                                      </span>
                                    )}
                                    {willChangeCv && (
                                      <span className="inline-flex items-center gap-0.5 sm:gap-1 text-[10px] sm:text-xs text-orange-600 bg-orange-50 px-1.5 sm:px-2 py-0.5 rounded-full animate-pulse">
                                        <FaExclamationTriangle size={8} /> Will recalculate ATS
                                      </span>
                                    )}
                                  </div>
                                </div>
                              ...
                              <a
                                ...
                                className="text-blue-600 hover:text-blue-800 text-xs sm:text-sm font-medium px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg hover:bg-blue-50 transition-colors ml-10 sm:ml-0 mt-2 sm:mt-0"
                    ...
                    {errors.cv_id && (
                      <p className="text-red-500 text-xs sm:text-sm mt-1.5 sm:mt-2 flex items-center gap-1">
                        <FaExclamationTriangle size={10} /> {errors.cv_id}
                      </p>
                    )}
                    ...
                    {cvs.length > 0 && formData.cv_id && parseInt(formData.cv_id) !== parseInt(originalCvId) && !showAtsPreview && (
                      <div className="mt-3 sm:mt-4">
                        <button
                          type="button"
                          onClick={handlePreviewAts}
                          className="inline-flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm text-purple-600 hover:text-purple-700 font-medium px-3 sm:px-4 py-1.5 sm:py-2 bg-purple-50 hover:bg-purple-100 rounded-xl transition-all duration-200 border border-purple-200"
                        >
                          <FaChartLine size={12} />
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-5">
                    <div>
                      ...
                      <input
                        ...
                        className={`w-full px-3 sm:px-4 py-2 sm:py-3 border-2 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 text-sm sm:text-base ${errors.name ? 'border-red-400 bg-red-50' : 'border-gray-200 hover:border-blue-300'
                          }`}
                        placeholder="John Doe"
                      />
                      {errors.name && (
                        <p className="text-red-500 text-[10px] sm:text-xs mt-1 flex items-center gap-1">
                          <FaExclamationTriangle size={8} /> {errors.name}
                        </p>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={handleChange}
                        className={`w-full px-3 sm:px-4 py-2 sm:py-3 border-2 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 text-sm sm:text-base ${errors.email ? 'border-red-400 bg-red-50' : 'border-gray-200 hover:border-blue-300'
                          }`}
                        placeholder="john@example.com"
                      />
                      {errors.email && (
                        <p className="text-red-500 text-[10px] sm:text-xs mt-1 flex items-center gap-1">
                          <FaExclamationTriangle size={8} /> {errors.email}
                        </p>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={handleChange}
                        className={`w-full px-3 sm:px-4 py-2 sm:py-3 border-2 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 text-sm sm:text-base ${errors.phone ? 'border-red-400 bg-red-50' : 'border-gray-200 hover:border-blue-300'
                          }`}
                        placeholder="+880 1234 567890"
                      />
                      {errors.phone && (
                        <p className="text-red-500 text-[10px] sm:text-xs mt-1 flex items-center gap-1">
                          <FaExclamationTriangle size={8} /> {errors.phone}
                        </p>
                      )}
                    </div>
                    ...
                    {showSalaryInput() && (
                      <div>
                        ...
                        <input
                          ...
                          onChange={handleChange}
                          className={`w-full px-3 sm:px-4 py-2 sm:py-3 border-2 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 text-sm sm:text-base ${errors.expected_salary ? 'border-red-400 bg-red-50' : 'border-gray-200 hover:border-blue-300'
                            }`}
                          placeholder={getSalaryPlaceholder()}
                        />
                        {errors.expected_salary && (
                          <p className="text-red-500 text-[10px] sm:text-xs mt-1 flex items-center gap-1">
                            <FaExclamationTriangle size={8} /> {errors.expected_salary}
                          </p>
                        )}
                  ...
                  {(jobListing.required_linkedin_link || jobListing.required_facebook_link) && (
                    <div className="space-y-3 sm:space-y-4 p-3 sm:p-4 bg-gray-50 rounded-2xl border border-gray-200">
                      <p className="text-xs sm:text-sm font-medium text-gray-700">Social Media Profiles</p>
                      {jobListing.required_linkedin_link && (
                        <div>
                          ...
                          <input
                            ...
                            onChange={handleChange}
                            className={`w-full px-3 sm:px-4 py-2 sm:py-3 border-2 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 text-sm sm:text-base ${errors.linkedin_link ? 'border-red-400 bg-red-50' : 'border-gray-200 hover:border-blue-300'
                              }`}
                            placeholder="https://linkedin.com/in/username"
                          />
                          {errors.linkedin_link && (
                            <p className="text-red-500 text-[10px] sm:text-xs mt-1 flex items-center gap-1">
                              <FaExclamationTriangle size={8} /> {errors.linkedin_link}
                            </p>
                          )}
                        </div>
                      )}
                      ...
                      {jobListing.required_facebook_link && (
                        <div>
                          ...
                          <input
                            ...
                            onChange={handleChange}
                            className={`w-full px-3 sm:px-4 py-2 sm:py-3 border-2 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 text-sm sm:text-base ${errors.facebook_link ? 'border-red-400 bg-red-50' : 'border-gray-200 hover:border-blue-300'
                              }`}
                            placeholder="https://facebook.com/username"
                          />
                          {errors.facebook_link && (
                            <p className="text-red-500 text-[10px] sm:text-xs mt-1 flex items-center gap-1">
                              <FaExclamationTriangle size={8} /> {errors.facebook_link}
                            </p>
                          )}
                        </div>
                      )}
                  ...
                  {application.ats_score !== null && application.ats_calculation_status === 'completed' && (
                    <div className={`rounded-2xl p-3 sm:p-4 flex flex-col sm:flex-row items-start gap-2 sm:gap-3 border ${getAtsScoreBg(application.ats_score).replace('bg-', 'border-').replace('100', '200')}`}>
                      <div className={`p-1.5 sm:p-2 rounded-xl ${getAtsScoreBg(application.ats_score)}`}>
                        <FaStar className={getAtsScoreColor(application.ats_score)} size={14} />
                      </div>
                  ...
                  {application.ats_calculation_status === 'processing' && (
                    <div className="bg-yellow-50 rounded-2xl p-3 sm:p-4 flex flex-col sm:flex-row items-start gap-2 sm:gap-3 border border-yellow-200">
                      <div className="p-1.5 sm:p-2 bg-yellow-100 rounded-xl">
                        <FaSpinner className="animate-spin text-yellow-600" size={14} />
                      </div>
                      <div className="text-xs sm:text-sm text-yellow-800">
                        <p className="font-medium mb-0.5 sm:mb-1">ATS Score is being calculated</p>
                        <p className="text-[10px] sm:text-xs">Please check back in a few moments.</p>
                      </div>
                    </div>
                  )}

                  {application.ats_calculation_status === 'failed' && (
                    <div className="bg-red-50 rounded-2xl p-3 sm:p-4 flex flex-col sm:flex-row items-start gap-2 sm:gap-3 border border-red-200">
                      <div className="p-1.5 sm:p-2 bg-red-100 rounded-xl">
                        <FaExclamationTriangle className="text-red-600" size={14} />
                      </div>
                      <div className="text-xs sm:text-sm text-red-800">
                        <p className="font-medium mb-0.5 sm:mb-1">ATS Score calculation failed</p>
                        <p className="text-[10px] sm:text-xs">Saving changes will trigger a recalculation.</p>
                      </div>
                    </div>
                  )}

                  {/* Info Note */}
                  <div className="bg-linear-to-r from-blue-50 to-indigo-50 rounded-2xl p-4 sm:p-5 border border-blue-100">
                    <div className="flex flex-col sm:flex-row items-start gap-2 sm:gap-3">
                      ...
                      <div className="text-xs sm:text-sm text-blue-800 flex-1">
                        <p className="font-semibold mb-1.5 sm:mb-2">Important Notes</p>
                ...
                {/* Form Actions */}
                <div className="px-4 sm:px-6 py-3 sm:py-4 bg-gray-50 border-t border-gray-100 flex flex-col sm:flex-row flex-wrap justify-end gap-2 sm:gap-3">
                  <button
                    type="button"
                    onClick={() => router.visit(route('backend.apply.show', application.id))}
                    className="w-full sm:w-auto px-4 sm:px-6 py-2 sm:py-2.5 text-gray-700 bg-white border-2 border-gray-200 rounded-xl hover:bg-gray-50 hover:border-gray-300 transition-all duration-200 font-medium text-sm"
                  >
                    Cancel
                  </button>
                  <button
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              <div className="bg-white rounded-2xl shadow-lg overflow-hidden sticky top-24 border border-gray-100">
                <div className="px-4 sm:px-6 py-3 sm:py-4 bg-linear-to-r from-blue-600 to-indigo-600">
                  <h3 className="font-semibold text-white flex items-center gap-1.5 sm:gap-2 text-sm sm:text-base">
                ...
                <div className="p-4 sm:p-5 space-y-2.5 sm:space-y-3">
                  <div className="flex justify-between items-center py-1.5 sm:py-2 border-b border-gray-100">
                    <span className="text-xs sm:text-sm text-gray-500">Application ID</span>
                    <span className="text-xs sm:text-sm font-semibold text-gray-900">#{application.id}</span>
                  </div>
                  <div className="flex justify-between items-center py-1.5 sm:py-2 border-b border-gray-100">
                    <span className="text-xs sm:text-sm text-gray-500">Submitted On</span>
                    <span className="text-xs sm:text-sm font-medium text-gray-900">{formatDate(application.created_at)}</span>
                  </div>
                  <div className="flex justify-between items-center py-1.5 sm:py-2 border-b border-gray-100">
                    <span className="text-xs sm:text-sm text-gray-500">Status</span>
                    <span className="inline-flex items-center gap-1 px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-medium bg-yellow-100 text-yellow-800">
                      <FaClock size={8} />
                      Pending
                    </span>
                  </div>
                  {application.ats_score && application.ats_calculation_status === 'completed' && (
                    <div className="flex justify-between items-center py-1.5 sm:py-2">
                      <span className="text-xs sm:text-sm text-gray-500">Current ATS Score</span>
                      <span className={`text-xs sm:text-sm font-bold ${getAtsScoreColor(application.ats_score)}`}>
                        {Math.round(application.ats_score)}%
                      </span>
                    </div>
                  )}
              ...
              <div className="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100">
                ...
                <div className="p-4 sm:p-5 space-y-3 sm:space-y-4">
                  {[
                    ...
                    <div key={tip.num} className="flex items-start gap-2.5 sm:gap-3">
                      <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center shrink-0 text-xs sm:text-sm font-bold">
                        {tip.num}
                      </div>
                      <div>
                        <p className="font-medium text-gray-900 text-xs sm:text-sm">{tip.title}</p>
                        <p className="text-[10px] sm:text-xs text-gray-500">{tip.desc}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {/* Warning Card */}
              <div className="bg-linear-to-r from-yellow-50 to-orange-50 rounded-2xl p-4 sm:p-5 border border-yellow-200">
